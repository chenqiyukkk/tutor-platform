...
WHERE "status" = 'PUBLISHED'
  ...
  AND "bio" IS NOT NULL
  AND btrim("bio") <> ''
  AND NOT "public_content_unsafe_v1"("displayName")
  AND NOT "public_content_unsafe_v1"("headline")
  AND NOT "public_content_unsafe_v1"("bio");

UPDATE "TutoringRequest" AS request
SET "publicContentSafetyVersion" = 1
WHERE request."status" = 'PUBLISHED'
  AND request."publishedAt" IS NOT NULL
  AND btrim(request."title") <> ''
  AND btrim(request."description") <> ''
  AND NOT "public_content_unsafe_v1"(request."title")
  AND NOT "public_content_unsafe_v1"(request."description")
  AND NOT "public_content_unsafe_v1"(request."schedule")
  AND NOT "public_content_unsafe_v1"(request."publicLocationNote")
  AND EXISTS (
    SELECT 1
    FROM "StudentProfile"
    WHERE "StudentProfile"."id" = request."studentProfileId"
