...
- While the migration holds the `Message` table lock, backfill existing rows directly with sequence `nextval`, make the column non-null, replace the superseded `(conversationId, updatedAt, id)` index with `(conversationId, changeVersion)`, and install the pair-locking trigger last. This keeps the migration atomic without creating a table-lock → pair-lock edge against application transactions.
