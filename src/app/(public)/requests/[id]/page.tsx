...
import { getAdjacentRegionPairs, getDirectoryViewerContext } from "@/features/directory/personalization";
...
export default async function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const request = await directoryRepository.getRequest(parsed.data);
  if (!request) notFound();
    ...
    <main className="directory-detail directory-detail--request" id="main-content">
      ...
      <div className="site-container directory-detail__grid">
        <article className="directory-profile-sheet">
          ...
          <section><p className="eyebrow">需求说明</p><h2>希望老师了解的学习情况</h2><p className="directory-detail__prose">{request.description}</p></section>
          <section><p className="eyebrow">安排与预算</p><dl className="directory-detail__facts"><div><dt>预算</dt><dd>{budget(request.budgetMinCents, request.budgetMaxCents)}</dd></div><div><dt>上课方式</dt><dd>{request.teachingMode ? modes[request.teachingMode] : "待商量"}</dd></div><div><dt>时间安排</dt><dd>{request.scheduleText ?? "待商量"}</dd></div><div><dt>大致位置</dt><dd>{request.region?.name}{request.publicLocationNote ? ` · ${request.publicLocationNote}` : ""}</dd></div></dl></section>
        ...
        <DirectoryContactCta isLoggedIn={Boolean(viewer)} kind="request" />
      </div>
    </main>
  );
}
