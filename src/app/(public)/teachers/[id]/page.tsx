...
export default async function TeacherDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const teacher = await directoryRepository.getTeacher(parsed.data);
  if (!teacher) notFound();
    ...
    <main className="directory-detail" id="main-content">
      ...
      <div className="site-container directory-detail__grid">
        <article className="directory-profile-sheet">
          ...
          <section><p className="eyebrow">教学自述</p><h2>怎样陪学生把问题想明白</h2><p className="directory-detail__prose">{teacher.bio}</p></section>
        ...
        <DirectoryContactCta isLoggedIn={Boolean(viewer)} kind="teacher" />
      </div>
    </main>
  );
}
