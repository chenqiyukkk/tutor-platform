...
const contactPatterns = [
  ...
  /小\s*红\s*书|xiao\s*hong\s*shu/iu,
  /(?:^|[^a-z])red(?=\s*(?:号|id|[:：@]))/iu,
  /抖\s*音|dou\s*yin|tik\s*tok/iu,
  /(?:^|[^a-z])line(?=\s*(?:号|id|[:：@]))/iu,
  /(?:^|[^a-z])signal(?=\s*(?:号|id|[:：@]))/iu,
  /二\s*维\s*码|扫码/iu,
  /[\p{L}\p{N}._%+-]+@[\p{L}\p{N}.-]+\.[\p{L}]{2,}/iu,
  /(?:https?:\/\/|www\.)\S+/iu,
  /\b(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:com|cn|net|org|io|co|me|app|dev|xyz|top|site|online|tech|edu|gov|info|biz|club|pro|cc|tv)\b/iu,
